...
// т.к. имееют низкую абстракцию и напрямую вызываться не должны. 
// Чтобы получить к ним доступ нужно обращаться напрямую к src.

// Файл TRS является исключением, поскольку используется 
// не только ядром, но и более верхними абстракциями (visuals).
